...
public class ReviewController {
	...
	public ResponseEntity<Review> save (@RequestBody final Review contatoEntity){
		return new ResponseEntity<Review>(reviewDAO.save(contatoEntity), HttpStatus.CREATED);
	}
	
	@CrossOrigin
	@RequestMapping(value="/{id}", method = RequestMethod.PUT)	
	public ResponseEntity<Review> alter (@PathVariable("id") int id, @RequestBody final Review contatoEntity) {
		return new ResponseEntity<Review>(reviewDAO.save(contatoEntity), HttpStatus.OK);
	}
